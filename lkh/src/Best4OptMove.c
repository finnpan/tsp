...
{
    Node *t3, *t4, *t5, *t6 = 0, *t7, *t8 = 0, *T3 = 0, *T4 = 0, *T5 = 0,
                        *T6 = 0, *T7 = 0, *T8 = 0;
        ...
        for (X4 = ProblemType == ATSP ? 2 : 1; X4 <= 2; X4++) {
            ...
            for (Nt4 = t4->CandidateSet; (t5 = Nt4->To); Nt4++) {
                ...
                for (X6 = 1; X6 <= 2; X6++) {
                    ...
                    if (Fixed(t5, t6)) continue;
                    G4 = G3 + C(t5, t6);
                    if ((Case6 <= 2 || Case6 == 5 || Case6 == 6)
                        && !Forbidden(t6, t1) && (!c || G4 - c(t6, t1) > 0)
                        && (*Gain = G4 - C(t6, t1)) > 0) {
                    ...
                    for (Nt6 = t6->CandidateSet; (t7 = Nt6->To); Nt6++) {
                        ...
                        for (X8 = 1; X8 <= 2; X8++) {
                            if (X8 == 1) {
                                ...
                                switch (Case6) {
                                    case 1:
                                        t8 = BETWEEN(t2, t7, t5) ? SUC(t7)
                                                                 : PRED(t7);
                                        break;
                                    case 2:
                                        t8 = BETWEEN(t3, t7, t6) ? SUC(t7)
                                                                 : PRED(t7);
                                        break;
                                    case 3:
                            ...
                            if (t8 == t1 || (t7 == t3 && t8 == t4)
                                || (t7 == t4 && t8 == t3))
                                continue;
                            if (Fixed(t7, t8)) continue;
                            G6 = G5 + C(t7, t8);
                            if (t8 != t1 && !Forbidden(t8, t1)
                                && (!c || G6 - c(t8, t1) > 0)
                                && (*Gain = G6 - C(t8, t1)) > 0) {
                                Make4OptMove(t1, t2, t3, t4, t5, t6, t7, t8,
                                             Case8);
                                return 0;
                            }
                            if (GainCriterionUsed && G6 - Precision < t8->Cost)
                                continue;
                            if (!Backtracking || Swaps > 0) {
                                if ((G6 > BestG6
                                    ...
                                    /* Ignore the move if the gain does
                                       not vary */
                                    if (RestrictedSearch && ProblemType != HCP
                                        && ProblemType != HPP
                                        && G2 - t4->Pi == G4 - t6->Pi
                                        && G4 - t6->Pi == G6 - t8->Pi
                                        && G3 + t5->Pi == G1 + t3->Pi
                                ...
                                Exclude(t7, t8);
                                while (
                                    (t = BestSubsequentMove(t1, t, &G, Gain)))
                                    ;
                                if (*Gain > 0) return 0;
                                RestoreTour();
