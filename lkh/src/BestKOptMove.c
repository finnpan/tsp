...
static GainType BestKOptMoveRec(int k, GainType G0)
{
    ...
    for (Nt2 = t2->CandidateSet; (t3 = Nt2->To); Nt2++) {
        ...
        for (X4 = 1; X4 <= 2; X4++) {
            ...
            if (Fixed(t3, t4) || Deleted(t3, t4)) continue;
            t[2 * k] = t4;
            G2 = G1 + C(t3, t4);
            G3 = MINUS_INFINITY;
            ...
            if (t4 != t1 && !Forbidden(t4, t1) && k + 1 < NonsequentialMoveType
                ...
                if ((PatchingCRestricted ? G3 > 0 && IsCandidate(t4, t1)
                     : PatchingCExtended ? G3 > 0 || IsCandidate(t4, t1)
                                         : G3 > 0)
                ...
                if (!Backtracking || Swaps > 0) {
                    if ((G2 > BestG2
                         || (G2 == BestG2 && !Near(t3, t4)
                             && Near(T[2 * K - 1], T[2 * K])))
                        && Swaps < MaxSwaps && Excludable(t3, t4)) {
                        if (RestrictedSearch && K > 2 && ProblemType != HCP
                            && ProblemType != HPP) {
        ...
        for (i = 2 * k - 4; i >= 2; i--) {
            if (t3 == t[i]) {
                t4 = t[i ^ 1];
                if (t4 == t1 || Forbidden(t4, t1) || Fixed(t3, t4)
                    || Added(t4, t1))
                    continue;
                G2 = G1 + C(t3, t4);
